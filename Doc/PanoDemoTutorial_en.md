...
**Note: For the DJI Inspire 1 and the Phantom 3 Professional, using the DJI SDK and OpenCV**
...
With the Inspire 1, we just rotated the gimbal 360 degrees, taking 8 photos along the way. However, the Phantom 3 Professional's gimbal does not have the capabilities to rotate 360 degrees. This is as in some of the angles its legs would be visible, while the Inspire 1's legs lift up out of the field of view. Consequently, instead of rotating the gimbal to take photos, we're going to have to rotate the entire drone.

Firstly, though, we're going to add some code so that we can execute a different algorithm when we are connected to a Phantom 3 Professional. One of the global variables I had you declare was a **DJIDroneType** object **mDroneType**. In **startDJICamera()**, a function called when the start button is pressed, we execute this line:
...
Now if your app is connected to a Phantom 3 Professional, when **stitchingButton** is pressed, the app will send the handler a **HANDLER_PHANTOM3PROFESSIONAL_CAPTURE_IMAGES**.
...
There we go! Your automated panorama app is now complete, ready to use on both the Inspire 1 and Phantom 3 Professional! Run your code and have fun!
